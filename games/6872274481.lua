...
local function addBlur(parent)
	...
	blur.Image = getcustomasset('rendervape/assets/new/blur.png');
...
run(function()
	...
	sessioninfo:AddItem('Map', 0, function()
		return string.gsub(string.split((workspace:WaitForChild('Map', 10):WaitForChild('Worlds', 5):GetChildren()[1].Name), '_')[2] or (workspace:WaitForChild('Map', 10):WaitForChild('Worlds', 5):GetChildren()[1].Name), '-', '') or 'Blank'
	end, false)

	local Knit = {Controllers = {}, GetController = function(self, Name: string)
	...
	function bedwars.switchHand(item: Instance)
		if not item then
			return
		end

		if lplr.Character.HandInvItem.Value ~= item then
			lplr.Character.HandInvItem.Value = item
		...
		if Name == 'SwordHit' then
			return {
				...
				SendToServer = function(self, data: {}, ...)
					...
					if (Reach and Reach.Enabled) or (HitBoxes and HitBoxes.Enabled) then
...
bedwars.getIcon = function(data: {itemType: string})
	for ind, val in Images.ImageIds do
		if ind == data.itemType then
			return val
		end
	end

	print('not found?', data.itemType)
	return ''
end

run(function()
	local old = bedwars.Knit.Controllers.SprintController.stopSprinting;

	vape.Categories.Combat:CreateModule({
		Name = 'Sprint',
		Function = function(callback)
			if callback then
				bedwars.Knit.Controllers.SprintController:startSprinting()
				bedwars.Knit.Controllers.SprintController.stopSprinting = function() end
			else
				bedwars.Knit.Controllers.SprintController.stopSprinting = old
				bedwars.Knit.Controllers.SprintController:stopSprinting()
			end
		end
	})
end)
...
run(function()
	...
	AntiFall = vape.Categories.Blatant:CreateModule({
		...
		Function = function(callback)
			if callback then
				repeat task.wait() until bedwars.Knit.Controllers.MatchController:getMatchState() ~= 0 or (not AntiFall.Enabled)
			...
			if callback then
				...
				Fly:Clean(runService.PreSimulation:Connect(function(dt)
					if entitylib.isAlive and isnetworkowner(entitylib.character.RootPart) then
						local flyAllowed = (lplr.Character:GetAttribute('InflatedBalloons') and lplr.Character:GetAttribute('InflatedBalloons') > 0) or bedwars.Knit.Controllers.MatchController:getMatchState() == 2
...
local function getSword()
	for ind, val in InventoryUtil.getInventory(lplr).items do
		if val.itemType:lower():find('sword') then
			return val
		end
	end
end
	...
	local Updated = {
		Normal = function(ent)
			...
			if nametag then
				...
				local inventory = InventoryUtil.getInventory(ent.Player);
				if Equipment.Enabled and inventory then
...
run(function()
	...
	AutoVoidDrop = vape.Categories.Utility:CreateModule({
		...
		Function = function(callback)
			if callback then
				repeat task.wait() until bedwars.Knit.Controllers.MatchController:getMatchState() ~= 0 or (not AutoVoidDrop.Enabled)
		...
		Function = function(callback)
			if callback then
				...
				if (not Skywars.Enabled) or tostring(store.queueType):find('skywars') then
					repeat
						if entitylib.isAlive and bedwars.Knit.Controllers.MatchController:getMatchState() ~= 2 then
...
run(function()
	...
	local function addItem(itemType, shop)
		...
		item.Image = bedwars.getIcon(itemType)
			...
			if callback then
				DamageIndicator:Clean(workspace.ChildAdded:Connect(function(child: Instance)
					if child.Name == 'DamageIndicatorPart' then
						...
						child.BillboardGui.Frame.TextLabel.Text = (#CustomText.ListEnabled > 0 and CustomText.ListEnabled[math.random(1, #CustomText.ListEnabled)] or child.BillboardGui.Frame.TextLabel.Text)
...
local function getWool()
	for index, value in InventoryUtil.getInventory(lplr).items do
		if value.itemType:lower():find('wool') then
			return value;
		end
	end
end
...
run(function()
	...
	Scaffold = vape.Categories.World:CreateModule({
		Name = 'Scaffold',
		Function = function(callback)
			if callback then
				repeat
					...
					if Wool then
						task.spawn(function()
							--[[bedwars.Knit.Controllers.BlockController:placeBlock({
								itemType = Wool.itemType,
								position = (
									entitylib.character.RootPart.CFrame.Position +
									(entitylib.character.Humanoid.MoveDirection * (Expand.Value * 3)) -
									Vector3.new(0, 4, 0)
								)
							})]]

							for ind = 1, Expand.Value do
								...
								bedwars.Knit.Controllers.BlockController:placeBlock({
									...
									position = (
										entitylib.character.RootPart.CFrame.Position +
										(inputService:IsKeyDown(Enum.KeyCode.Space) and (entitylib.character.Humanoid.MoveDirection * 2) or (entitylib.character.Humanoid.MoveDirection * (ind * 3))) -
										Vector3.new(0, 4, 0)
									)
								})
								task.wait(0.03)
						...
						if Tower.Enabled and inputService:IsKeyDown(Enum.KeyCode.Space) then
							entitylib.character.RootPart.Velocity = Vector3.new(entitylib.character.RootPart.Velocity.X, TowerSpeed.Value, entitylib.character.RootPart.Velocity.Z)
						end
					end
	...
	Tower = Scaffold:CreateToggle({
		Name = 'Tower',
		Function = function(callback)
			TowerSpeed.Object.Visible = callback
		end
	})
	TowerSpeed = Scaffold:CreateSlider({
		Name = 'Tower Speed',
		Min = 1,
		Max = 40,
		Default = 30,
	})
	TowerSpeed.Object.Visible = false
end)
