...
local function rakNetCheck(module)
	if not (raknet and raknet.add_send_hook and pcall(raknet.add_send_hook, function() end)) then
		vape:CreateNotification(module, 'This feature requires raknet! (risky feature, please do not use on mains.)', 10, 'warning')
		return false
	end
...
local hash = loadstring(downloadFile('rendervape/libraries/hash.lua'), 'hash')()
local prediction = loadstring(downloadFile('rendervape/libraries/prediction.lua'), 'prediction')()
entitylib = loadstring(downloadFile('rendervape/libraries/entity.lua'), 'entitylibrary')()
local whitelist = {
	alreadychecked = {},
	customtags = {},
...
run(function()
	...
	function whitelist:update(first)
		...
		if not first or whitelist.textdata ~= whitelist.olddata then
			if not first then
				whitelist.olddata = isfile('rendervape/profiles/whitelist.json') and readfile('rendervape/profiles/whitelist.json') or nil
			end
			...
			if whitelist.textdata ~= whitelist.olddata then
				...
				pcall(function()
					writefile('rendervape/profiles/whitelist.json', whitelist.textdata)
				end)
			end
...
run(function()
	...
	Mode = Phase:CreateDropdown({
		Name = 'Mode',
		List = {'Part', 'Character', 'CFrame', 'Motor'},
		Function = function(val)
...
run(function()
	...
	local function Added(ent)
		...
		arrow.Visible = false
		arrow.Image = getcustomasset('rendervape/assets/new/arrowmodule.png')
...
run(function()
	...
	Radar = vape:CreateOverlay({
		Name = 'Radar',
		Icon = getcustomasset('rendervape/assets/new/radaricon.png'),
		...
		Function = function(callback)
			if callback then
				if Mode.Value == 'Workspace' then
					...
					Gravity:Clean(runservice.PreSimulation:Connect(function()
						if vape.Modules.InfiniteFly and vape.Modules.InfiniteFly.Enabled then return end
						workspace.Gravity = Value.Value
					end))
				else
